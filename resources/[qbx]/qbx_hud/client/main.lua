...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    Wait(2000)
    -- Sempre aplica config do servidor (ignora preferencias salvas do player)
    loadSettings(sharedConfig.menu)
    SetResourceKvp('hudSettings', json.encode(sharedConfig.menu))
...
local function setNitroActive(vehicle, active, mode)
    ...
    if vehicle and vehicle ~= 0 and DoesEntityExist(vehicle) then
        ...
        if active then
            ...
            if not wasActive or not visual or visual.mode ~= activeMode then
