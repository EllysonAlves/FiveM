...
function displayInfoModal(title, message) {
    ...
    setHtml('#scoreboard-tbody', `<tr><td colspan="7" style="text-align:center;">${message}</td></tr>`);
...
function displayScoreboard(data) {
    ...
    if (Array.isArray(data?.times) && data.times.length) {
        ...
        html = '<tr><td colspan="7" style="text-align:center;">Nenhuma corrida registrada ainda</td></tr>';
    }
